        ...
        reasons.append(
            ReasonItem(
                ...
                detail=(
                    ...
                    if not resolution.isDirectional
                    else "No exact packaged-food match yet. The scan is staying directional instead of inventing certainty."
                ),
                impact=ReasonImpact.positive if not resolution.isDirectional else ReasonImpact.caution,
            )
        )
...
def build_warnings(product: ProductCandidate, tokens: list[str], overall_score: int) -> list[str]:
    warnings: list[str] = []
    resolution = product.resolution
    if resolution is not None and resolution.isDirectional:
...
def summary_for_score(score: int, product: ProductCandidate) -> str:
    if product.resolution and product.resolution.isDirectional:
...
def build_why_today(analysis: ScanAnalysis, recent_checkins: list[CheckInEvent], source: ScanSource) -> list[str]:
    ...
    if analysis.resolvedProduct.resolution is not None:
        reasons.append(
            ...
            if not analysis.resolvedProduct.resolution.isDirectional
...
def build_recommended_actions(overall: int, product: ProductCandidate) -> list[str]:
    if product.resolution is not None and product.resolution.isDirectional:
        return [
